...
export async function isAreaCleaned(input_x: number, input_y: number): Promise<boolean> {
    try {
        const data = await pool.query(
            `SELECT is_cleaned FROM grid_coordinates WHERE x = $1 AND y = $2 UNION ALL SELECT is_cleaned FROM grid_coordinates_partition1 WHERE x = $1 AND y = $2 UNION ALL SELECT is_cleaned FROM grid_coordinates_partition2 WHERE x = $1 AND y = $2`,
            [input_x, input_y]
        );
...
export async function updateAreaToClean(input_x: number, input_y: number): Promise<void> {
    try {
        await pool.query(
            `UPDATE grid_coordinates SET is_cleaned = TRUE WHERE x = $1 AND y = $2; UPDATE grid_coordinates_partition1 SET is_cleaned = TRUE WHERE x = $1 AND y = $2; UPDATE grid_coordinates_partition2 SET is_cleaned = TRUE WHERE x = $1 AND y = $2;`,
            [input_x, input_y]
        );
